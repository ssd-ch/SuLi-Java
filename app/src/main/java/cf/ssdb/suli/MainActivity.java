...
public class MainActivity extends AppCompatActivity {
                ...
                try {
                    ...
                    for (int i = 0; i < urlList.size(); i++) {
                        try {
                            ...
                            value[i][1] = urlList.get(i).text().replace("教室配当表_", "").replaceAll("_", " ");
                     ...
                     */
                    Elements FormData = Jsoup.connect(SyllabusFormUrl).get().select("table");
    ...
    private String[][] CreateCancelInfo(String url) throws IOException{
        ...
        do {
            ...
            //そのまま:tag #:id .:class
            if (document.select(".prevnextpage").size() < (page_num++ == 1 ? 1 : 2)) flg = false;
        } while (flg);
